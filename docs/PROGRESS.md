...
| 1.1.0 skins | M14 browser run 1 — **fail** | 732 | the new skin matrix, written before the fix: `"macos" paints identically to vscode` and the same for chrome, slate, nord, obsidian, tokyo — 104 selectors keyed on an attribute nothing set |
| 1.1.0 skins | M14 browser run 2 — **fail** | 732 | after the CSS rename *alone*: `panel is rgba(30,30,40,0.45) in both schemes — the skin's own tokens are shadowing the light ones`, for every skin. The half-fix caught by the half of the assertion that exists for it |
| **1.1.0 skins** | **M14 PASS** | **732** | `data-vdd-skin` + the colour scheme mirrored onto the workspace element. Seven built-in skins and the demo's own `mono` measured in both schemes on four surfaces; six sampled by hand as well (sidebar, toolbar, taskbar, window title bar and body, inner widget) — every one flips. 90 tokens documented and gated both ways; the attribute audit that would have caught this on day one is now a standing rule. M13, M11, M9, M1 PASS · selftest 86 rules · sweep 74 modules / 88% |
| 1.1.0 dark base | M14 run 1 — **fail** | 732 | the two new checks, written first: the gate rule named 29 tokens declared only in a scheme block, and the browser gate found `sidebarRail has no background in dark mode (rgba(0,0,0,0))`, same for the drawer and the toolbar |
| **1.1.0 dark base** | **M14 PASS** | **732** | the dark block folded into `:root` and removed — dark is the base look, so "absent means dark" is now true in CSS as well as in the contract. Rail `rgb(20,22,25)`/`rgb(233,236,239)`, drawer `rgb(30,32,36)`/`rgb(248,249,250)`, selected rail icon `rgb(56,189,248)`/`rgb(0,102,204)` where it previously had no colour at all. 119 tokens documented and gated. M9, M13, M1 PASS · selftest 87 · sweep 88% |
