...
**No commits, no `git init`.** The project has no git home yet, so nothing is committed
until one exists. The durable record of each gate is [PROGRESS.md](PROGRESS.md) plus the
artefacts under `artifacts/M<n>/`; a filesystem snapshot of each green state is written to
`artifacts/M<n>/tree.txt` (file list + hashes) so a regression can be located even without
history. Say the word once there is a repository and I will add per-milestone commits.
