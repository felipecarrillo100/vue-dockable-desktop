...
Gate: `spike/gate.mjs` (run from `spike/`, with `npx vite --port 5199` serving). Raw data: `result.json` · screenshot: `spike.png`

The raw output and screenshots named above are written to `artifacts/M0/` when the gate
